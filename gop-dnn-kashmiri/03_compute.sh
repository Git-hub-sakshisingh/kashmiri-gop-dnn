...
ln -fs $KALDI_ROOT/egs/gop_speechocean762/s5/local
...
if [ $stage -le 5 ]; then
    ...
    python3 scripts/generate_data_for_eval.py \
         --transcription-file $EPADB_ROOT/reference_transcriptions.txt \
         --reference-file $EPADB_ROOT \
         --gop-file exp_epadb/test_norm_original/gop.txt \
         --phones-pure-file exp_epadb/test_norm_original/align/phones-pure.txt \
         --labels-dir exp_epadb/labels \
         --output-dir exp_epadb/test_norm_original/gop_with_labels
...
# echo "Computing annotator agreement"

# python3 scripts/compute_annotator_agreement.py \
#    --train-root $EPADB_ROOT \
#    --output-dir $expdir/annotator_agreement
